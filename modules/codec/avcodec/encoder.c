...
static void probe_video_frame_rate( encoder_t *p_enc, AVCodecContext *p_context, AVCodec *p_codec )
{
    ...
    {
        p_context->time_base.num = p_enc->fmt_in.video.frame_rate.den;
        p_context->time_base.den = p_enc->fmt_in.video.frame_rate.num;
    }
    else
    {
        // MP4V doesn't like CLOCK_FREQ denominator in time_base, so use 1/25 as default for that
        p_context->time_base.num = 1;
        p_context->time_base.den = p_enc->fmt_out.i_codec == VLC_CODEC_MP4V ? 25 : CLOCK_FREQ;
    }
