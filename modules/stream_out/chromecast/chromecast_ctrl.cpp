...
int Open(vlc_object_t *p_module)
{
    ...
    if (p_renderer->target.psz_host == NULL)
    {
...
void vlc_renderer_sys::InputUpdated( input_thread_t *p_input )
{
    ...
    msg_Dbg( p_module, "InputUpdated p_input:%p was:%p device:%s", (void*)p_input, (void*)this->p_input, p_renderer->target.psz_host );
    ...
    if( this->p_input != NULL )
    {
        ...
        ssout << '#';
        ssout << "cc_sout{http-port=" << i_port << ",mux=" << muxer << ",mime=" << mime <<  ",video=" << (canDisplay==HAS_VIDEO ? '1' : '0') << "}";
