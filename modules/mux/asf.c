...
static block_t *asf_header_create( sout_mux_t *p_mux, bool b_broadcast )
{
    ...
    for( size_t i = 0; i < vlc_array_count( &p_sys->tracks ); i++ )
    {
        ...
        if( p_fmt->i_cat == VIDEO_ES && es_format_HasValidFrameRate( p_fmt ) )
    ...
    if( i_cm_size )
    {
        ...
        msg_Dbg( p_mux, "pixel aspect-ratio: %i/%i", dst.num, dst.den );
