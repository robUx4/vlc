...
float playlist_VolumeGet (playlist_t *pl)
{
    ...
    int ret = VLC_ENOOBJ;//vlc_renderer_volume_get( pl, &volume );
    if (ret == VLC_ENOOBJ)
    {
...
int playlist_VolumeSet (playlist_t *pl, float vol)
{
    ...
    int rret = VLC_ENOOBJ;//vlc_renderer_volume_set( pl, vol );
    audio_output_t *aout = playlist_GetAout (pl);
    if (aout != NULL)
    {
...
int playlist_VolumeUp (playlist_t *pl, int value, float *volp)
{
    ...
    float vol = playlist_VolumeGet( pl );
    if (vol >= 0.f)
    {
        vol += delta;
        if (vol < 0.f)
...
int playlist_MuteGet (playlist_t *pl)
{
    ...
    bool b_muted;
    int ret = VLC_ENOOBJ; //vlc_renderer_mute_get( pl, &b_muted );
    if (ret == VLC_SUCCESS)
        mute = b_muted;
...
int playlist_MuteSet (playlist_t *pl, bool mute)
{
    ...
    int rret = VLC_ENOOBJ; //vlc_renderer_mute_set( pl, mute );
    audio_output_t *aout = playlist_GetAout (pl);
    if (aout != NULL)
    {
