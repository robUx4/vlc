...
{
    vlc_mutex_lock( &p_resource->lock );
    vout_thread_t *p_ret = RequestVout( p_resource, p_vout, p_fmt, dpb_size, b_recycle, pf_pre_filter_cfg, pre_filter_cfg_opaque );
    vlc_mutex_unlock( &p_resource->lock );
...
{
    input_resource_RequestVout( p_resource, NULL, NULL, 0, false, NULL, NULL );
}
